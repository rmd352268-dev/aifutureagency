...
Please let me know the next steps & payment process.`;
  ...
  // =========================================================================
  // --- 15. PAYMENT GATEWAY CONTROLLER (bKash / Nagad / Rocket) ---
  // =========================================================================
  ...
  const payTrxIdInput = document.getElementById('payTrxIdInput');
  ...
  payMethodTabs.forEach(tab => {
    tab.addEventListener('click', () => {
      ...
      selectedPayMethod = tab.getAttribute('data-method');
    });
  });
  ...
  if (confirmPaymentBtn) {
    confirmPaymentBtn.addEventListener('click', () => {
      ...
      const enteredTrx = payTrxIdInput ? payTrxIdInput.value.trim() : '';
      ...
      try {
        ...
        localStorage.setItem('afa_admin_orders', JSON.stringify(existingOrders));
        localStorage.setItem('afa_latest_order_event', JSON.stringify({ time: Date.now(), order: newAdminOrder || newOrder }));
        try {
          if (typeof BroadcastChannel !== 'undefined') {
            const bc = new BroadcastChannel('afa_live_sync');
            bc.postMessage({ type: 'NEW_ORDER_PLACED', order: newAdminOrder || newOrder, timestamp: Date.now() });
          }
        } catch(err) {}
      } catch(e) {}
